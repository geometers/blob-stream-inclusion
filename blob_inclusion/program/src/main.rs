...
pub fn main() {
    ...
    let data_root: [u8; 32] = data_root.try_into().unwrap();
    ...
    let headers_vec = sp1_zkvm::io::read_vec();
    let headers: Vec<Header> = serde_cbor::from_slice(&headers_vec).unwrap();
    ...
    // Verify the row-inclusion range proof
    {
        ...
        assert!(range_proof
            .verify_range(&data_root, &blob_row_root_hashes,)
            .is_ok());
    }

    // Verify that the data root is included in the blobstream headers
    {
        let blobstream_data_hashes: Vec<_> = headers
            .iter()
            .map(|h| h.data_hash.unwrap())
            .collect();
        let data_root: Hash = data_root.to_vec().try_into().unwrap();
        assert!(blobstream_data_hashes.contains(&data_root));
    }

    let blobstream_commitment = compute_data_commitment(&headers);
    println!("blobstream_commitment: {:?}", B256::from_slice(&blobstream_commitment));
